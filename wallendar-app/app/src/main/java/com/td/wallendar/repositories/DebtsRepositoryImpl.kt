...
class DebtsRepositoryImpl(private val debtsService: DebtsService?) : DebtsRepository {
    override fun getTotalDebtsByUserId(userId: Long?): Single<MutableList<Debt?>?>? {
        return RetrofitUtils.performRequest(debtsService.getTotalDebtsByUserId(userId)).map(Function<MutableList<DebtResponse?>?, MutableList<Debt?>?> { obj: MutableList<DebtResponse?>? -> DebtMapper.toModel() })
    }
}