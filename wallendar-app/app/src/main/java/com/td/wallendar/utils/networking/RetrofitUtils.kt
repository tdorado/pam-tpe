...
object RetrofitUtils {
    fun <T> performRequest(request: Single<Response<T?>?>?): Single<T?>? {
        return request
                .onErrorResumeNext(Function<Throwable?, SingleSource<out Response<T?>?>?> { t: Throwable? -> Single.error(convertException(t)) })
    ...
    fun convertException(throwable: Throwable?): Throwable? {
        return (throwable as? UnknownHostException)?.let { NoConnectivityException(it) }
                ?: ((throwable as? IOException)?.let { RequestException(it) } ?: throwable)
    }
