...
class GroupBalanceViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(debt: Debt?, loggedUserId: Long) {
        ...
        val amount = df.format(debt.getAmount())
        if (debt.getFrom().id == loggedUserId) {
            remindButton.visibility = View.GONE
            val userToText = debt.getTo().firstName + " " + debt.getTo().lastName
            textView.text = itemView.context.getString(R.string.you_owe_to_user_amount, userToText, amount)
        } else if (debt.getTo().id == loggedUserId) {
            val userFromText = debt.getFrom().firstName + " " + debt.getFrom().lastName
        ...
        remindButton.setOnClickListener { view: View? -> onGroupRemindClickedListener.onGroupRemindClick(debt) }
        settleUpButton.setOnClickListener { view: View? -> onGroupSettleUpClickedListener.onGroupSettleUpClick(debt) }
    }
