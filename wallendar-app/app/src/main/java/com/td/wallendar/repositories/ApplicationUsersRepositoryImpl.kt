...
class ApplicationUsersRepositoryImpl(private val applicationUsersService: ApplicationUsersService?) : ApplicationUsersRepository {
    ...
    override fun createUser(addApplicationUserRequest: AddApplicationUserRequest?): Single<ApplicationUser?>? {
        return RetrofitUtils.performRequest(applicationUsersService.createUser(addApplicationUserRequest)).map(Function<ApplicationUserResponse?, ApplicationUser?> { obj: ApplicationUserResponse? -> ApplicationUserMapper.toModel() })
    }

    override fun getUser(userId: Long): Single<ApplicationUser?>? {
        return RetrofitUtils.performRequest(applicationUsersService.getUser(userId)).map(Function<ApplicationUserResponse?, ApplicationUser?> { obj: ApplicationUserResponse? -> ApplicationUserMapper.toModel() })
    }
    ...
    override fun getUserByEmail(email: String?): Single<ApplicationUser?>? {
        return RetrofitUtils.performRequest(applicationUsersService.getUserByEmail(email))
                .map(Function<ApplicationUserResponse?, ApplicationUser?> { obj: ApplicationUserResponse? -> ApplicationUserMapper.toModel() })
    }

    override fun createAlias(userId: Long, addUserAliasRequest: AddUserAliasRequest?): Single<UserAlias?>? {
        return RetrofitUtils.performRequest(applicationUsersService.createAlias(userId, addUserAliasRequest)).map(Function<UserAliasResponse?, UserAlias?> { obj: UserAliasResponse? -> UserAliasMapper.toModel() })
    }

    override fun getUserAliases(userId: Long): Single<MutableList<UserAlias?>?>? {
        return RetrofitUtils.performRequest(applicationUsersService.getUserAliases(userId)).map(Function<MutableList<UserAliasResponse?>?, MutableList<UserAlias?>?> { obj: MutableList<UserAliasResponse?>? -> UserAliasMapper.toModel() })
    }
}