...
class GroupMembersPresenter(groupMembersView: GroupMembersView?, groupsRepository: GroupsRepository?, schedulerProvider: SchedulerProvider?) {
    private val groupMembersView: WeakReference<GroupMembersView?>?
    ...
    private fun onGroupReceived(group: Group?) {
        groupMembersView.get().bind(ArrayList(group.getMembers()))
    }
    ...
    init {
        this.groupMembersView = WeakReference(groupMembersView)
