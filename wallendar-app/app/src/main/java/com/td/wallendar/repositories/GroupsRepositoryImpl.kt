...
class GroupsRepositoryImpl(val groupsService: GroupsService?) : GroupsRepository {
    override fun getGroupsByUser(userId: Long): Single<MutableList<Group?>?>? {
        return RetrofitUtils.performRequest(groupsService.getGroupsByUserId(userId)).map(Function<MutableList<GroupResponse?>?, MutableList<Group?>?> { obj: MutableList<GroupResponse?>? -> GroupMapper.toModel() })
    }

    override fun getGroup(groupId: Long): Single<Group?>? {
        return RetrofitUtils.performRequest(groupsService.getGroupById(groupId)).map(Function<GroupResponse?, Group?> { obj: GroupResponse? -> GroupMapper.toModel() })
    }

    override fun createGroup(addGroupRequest: AddGroupRequest?): Single<Group?>? {
        return RetrofitUtils.performRequest(groupsService.createGroup(addGroupRequest)).map(Function<GroupResponse?, Group?> { obj: GroupResponse? -> GroupMapper.toModel() })
    }

    override fun addMemberByEmail(groupId: Long, addMemberByEmailRequest: AddMemberByEmailRequest?): Single<Group?>? {
        return RetrofitUtils.performRequest(groupsService.addMemberByEmail(groupId, addMemberByEmailRequest)).map(Function<GroupResponse?, Group?> { obj: GroupResponse? -> GroupMapper.toModel() })
    }

    override fun addMembers(groupId: Long, addMembersRequest: AddMembersRequest?): Single<Group?>? {
        return RetrofitUtils.performRequest(groupsService.addMembers(groupId, addMembersRequest)).map(Function<GroupResponse?, Group?> { obj: GroupResponse? -> GroupMapper.toModel() })
    }

    override fun addPayment(groupId: Long, addPaymentRequest: AddPaymentRequest?): Single<Group?>? {
        return RetrofitUtils.performRequest(groupsService.addPayment(groupId, addPaymentRequest)).map(Function<GroupResponse?, Group?> { obj: GroupResponse? -> GroupMapper.toModel() })
    }
}