...
@Config(sdk = [30])
class AddMembersActivityTest {
    private var activity: ActivityController<AddMembersActivity?>? = null
    private var presenter: AddMembersPresenter? = null
    @Before
    fun setup() {
        MockitoAnnotations.initMocks(this)
        activity = Robolectric.buildActivity(AddMembersActivity::class.java)
        presenter = Mockito.mock(AddMembersPresenter::class.java)
    ...
    @Test
    fun whenSubmitMembersIsClicked_thenPresenterIsCalledAndActivityIsFinishedOnCallback() {
        Mockito.doNothing().`when`(presenter).submitMembers(1L, ArrayList())
        activity.get().findViewById<View?>(R.id.submit_members).performClick()
        Mockito.verify(presenter).submitMembers(1L, ArrayList())
